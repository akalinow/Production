...
bool HTauhTauhTree::pairSelection(unsigned int iPair){
  ...
  for(unsigned int iBit=0;iBit<HTTEvent::ntauIds;iBit++){
    if(HTTEvent::tauIDStrings[iBit]=="byTightIsolationMVArun2v1DBnewDMwLT2017v2")
      tauIDmask |= (1<<iBit);
    if(HTTEvent::tauIDStrings[iBit]=="byMediumIsolationMVArun2v1DBnewDMwLT2017v2")
      tauIDmaskMedium |= (1<<iBit);
    if(HTTEvent::tauIDStrings[iBit]=="byLooseIsolationMVArun2v1DBnewDMwLT2017v2")
  ...
  bool tauBaselineSelection1 = tau1P4.Pt()>35 && std::abs(tau1P4.Eta())<2.1 &&
                               daughters_decayModeFindingNewDMs->at(indexLeg1)>0.5 &&
  ...
  bool tauBaselineSelection2 = tau2P4.Pt()>35 && std::abs(tau2P4.Eta())<2.1 &&
                               daughters_decayModeFindingNewDMs->at(indexLeg2)>0.5 &&
...
unsigned int HTauhTauhTree::bestPair(std::vector<unsigned int> &pairIndexes){
  ...
  if(pairIndexes.size()) {
	...
	leg2Index = indexDau1->at(iPair);
      ...
      double iso_1_i = -getProperty("daughters_byIsolationMVArun2v1DBnewDMwLTraw2017v2",leg1Index);
      double iso_2_i = -getProperty("daughters_byIsolationMVArun2v1DBnewDMwLTraw2017v2",leg2Index);
  ...
  if(pairIndexes.size() && bestIndex!=(Int_t)pairIndexes[0]){
    ...
    std::cout<<"Pair sorting: "<<std::endl
	     ...
	     <<"\tiso1[best]="<<-iso_1<<", iso1[0]="<<getProperty("daughters_byIsolationMVArun2v1DBnewDMwLTraw2017v2",leg1Index)<<std::endl
	     <<"\tpt1[best]="<<sqrt(pt2_1)<<", pt1[0]="<<sqrt(pt2_1_i)<<std::endl
	     <<"\tiso2[best]="<<-iso_2<<", iso1[0]="<<getProperty("daughters_byIsolationMVArun2v1DBnewDMwLTraw2017v2",leg2Index)<<std::endl
